...
4. 右侧工具栏、Files、Browser、Side Chat 可打开，Files 可展开目录、切换隐藏文件并预览文件，且不会改动主会话。
5. 输入 `/` 可看到核心命令；模型/推理进入二级列表，状态卡可关闭和刷新，压缩/任务 fork 不会在仅浏览命令时误触发。
6. 未主动发送消息、安装扩展或创建任务时，冒烟检查不产生业务数据。
